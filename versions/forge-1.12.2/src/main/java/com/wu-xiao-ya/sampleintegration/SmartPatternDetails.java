...
public class SmartPatternDetails implements ICraftingPatternDetails {
    ...
    private final List<ItemStack> virtualInputStacks;
    private final List<ItemStack> virtualOutputStacks;
    private final String virtualFilterEntryId;
    ...
    public SmartPatternDetails(ItemStack patternStack) {
        ...
        this.virtualFilterEntryId = tag != null && tag.hasKey(TAG_VIRTUAL_FILTER_ENTRY_ID)
            ? tag.getString(TAG_VIRTUAL_FILTER_ENTRY_ID)
            : "";
    }
    ...
    public SmartPatternDetails(ItemStack patternStack, String specificInputOre, String specificOutputOre, String specificDisplayName) {
        ...
        this.outputCount = ItemTest.getOutputCountStatic(patternStack);
        this.isVirtual = true;
        this.virtualInputStacks = new ArrayList<>();
        this.virtualOutputStacks = new ArrayList<>();
        this.virtualFilterEntryId = "";
    }
    ...
    public boolean isValidItemForSlot(int slot, ItemStack itemStack, World world) {
        ...
        if (!ItemTest.isInputStackAllowedForPattern(patternStack, itemStack)) {
            return false;
        }
        if (slot < 0 || slot >= inputOreNames.size()) {
            return false;
        }
        if (isVirtual && virtualInputStacks != null && slot < virtualInputStacks.size()) {
            ItemStack expected = virtualInputStacks.get(slot);
            if (expected != null && !expected.isEmpty()) {
                return OreDictionary.itemMatches(expected, itemStack, false);
            }
        }
    ...
    public IAEItemStack[] getInputs() {
        List<ItemStack> inputs = buildStacksForOreList(inputOreNames, inputCounts, isVirtual, virtualInputStacks, true);
    ...
    public IAEItemStack[] getOutputs() {
        List<ItemStack> outputs = buildStacksForOreList(outputOreNames, outputCounts, isVirtual, virtualOutputStacks, false);
        List<IAEItemStack> resultList = new ArrayList<>();
    ...
    public ItemStack getOutput(InventoryCrafting inventory, World world) {
        ...
        if (isVirtual) {
            List<ItemStack> outputs = buildStacksForOreList(outputOreNames, outputCounts, true, virtualOutputStacks, false);
        ...
        ItemStack selectedOutput = selectFirstAllowedStack(outputItems, false);
    ...
    public boolean isAllowedByFilter() {
        ...
        String legacyId = inputOreName + "->" + outputOreName;

        boolean hasVariantEntriesForPair = false;
        String pairPrefix = legacyId + "|";
        ...
        boolean contains;
        if (virtualFilterEntryId != null && !virtualFilterEntryId.isEmpty() && hasVariantEntriesForPair) {
            contains = entries.contains(virtualFilterEntryId);
        } else {
            contains = entries.contains(legacyId);
            if (!contains && virtualFilterEntryId != null && !virtualFilterEntryId.isEmpty()) {
                contains = entries.contains(virtualFilterEntryId);
            }
        }
    ...
    public List<SmartPatternDetails> expandToVirtualPatterns() {
        ...
        if (!isWildcardPattern()) {
            if (hasAllowedStacksForCurrentPattern()) {
        ...
        for (String material : materials) {
            ...
            if (specificInputs.isEmpty() || specificOutputs.isEmpty()) {
                continue;
            }

            List<ItemStack> inputCandidates = OreDictionary.getOres(specificInputs.get(0));
            List<ItemStack> outputCandidates = OreDictionary.getOres(specificOutputs.get(0));
            boolean addedAny = false;
            boolean hadConcreteCandidate = false;
            Set<String> seenVariantIds = new HashSet<>();

            for (ItemStack inputCandidate : inputCandidates) {
                if (inputCandidate == null || inputCandidate.isEmpty()) {
                    continue;
                }
                hadConcreteCandidate = true;
                if (!ItemTest.isInputStackAllowedForPattern(patternStack, inputCandidate)) {
                    continue;
                }
                ItemStack outputCandidate = selectOutputForInput(inputCandidate, outputCandidates);
                if (outputCandidate == null || outputCandidate.isEmpty()) {
                    continue;
                }
                String variantId = buildVariantFilterEntryId(
                    specificInputs.get(0),
                    specificOutputs.get(0),
                    inputCandidate,
                    outputCandidate
                );
                ...
                List<ItemStack> concreteInputs = buildConcreteVirtualStacks(specificInputs, inputCounts, inputCandidate, true);
                List<ItemStack> concreteOutputs = buildConcreteVirtualStacks(specificOutputs, outputCounts, outputCandidate, false);
                if (concreteInputs.isEmpty() || concreteOutputs.isEmpty()) {
                    continue;
                }

                String virtualDisplay = inputCandidate.getDisplayName() + " -> " + outputCandidate.getDisplayName();
                ItemStack virtualStack = createVirtualPatternStack(
                    specificInputs,
                    specificOutputs,
                    concreteInputs,
                    concreteOutputs,
                ...
                virtualPatterns.add(new SmartPatternDetails(virtualStack));
                addedAny = true;
            }

            if (!addedAny) {
                if (hadConcreteCandidate) {
                    continue;
                }
                ItemStack fallback = createVirtualPatternStack(
                    specificInputs,
                    specificOutputs,
                    new ArrayList<>(),
                    new ArrayList<>(),
                    material + " " + displayName,
                    specificInputs.get(0) + "->" + specificOutputs.get(0)
                );
                virtualPatterns.add(new SmartPatternDetails(fallback));
            }
        }
    ...
    private boolean hasAllowedStacksForCurrentPattern() {
    ...
    private List<ItemStack> buildStacksForOreList(List<String> oreNames, List<Integer> counts, boolean virtual,
                                                  List<ItemStack> concreteVirtualStacks, boolean inputSide) {
        List<ItemStack> stacks = new ArrayList<>();
        if (oreNames.isEmpty()) {
            return stacks;
        }

        if (virtual && concreteVirtualStacks != null && !concreteVirtualStacks.isEmpty()) {
            int limit = Math.min(oreNames.size(), concreteVirtualStacks.size());
            for (int i = 0; i < limit; i++) {
                ItemStack base = concreteVirtualStacks.get(i);
                if (base == null || base.isEmpty()) {
                    continue;
                }
                if (!isStackAllowedForSide(base, inputSide)) {
                    continue;
                }
                ItemStack stack = base.copy();
                int count = i < counts.size() ? counts.get(i) : stack.getCount();
        ...
        for (int i = 0; i < oreNames.size(); i++) {
            ...
            ItemStack selected = selectFirstAllowedStack(items, inputSide);
            if (selected.isEmpty()) {
                continue;
            }
    ...
    private ItemStack selectOutputForInput(ItemStack inputCandidate, List<ItemStack> outputCandidates) {
        if (inputCandidate == null || inputCandidate.isEmpty() || outputCandidates == null || outputCandidates.isEmpty()) {
            return ItemStack.EMPTY;
        }

        ItemStack firstValid = ItemStack.EMPTY;
        ItemStack bestSameNamespace = ItemStack.EMPTY;
        String inputNamespace = getRegistryNamespace(inputCandidate);
        String inputPath = getRegistryPath(inputCandidate);
        int bestScore = -1;

        for (ItemStack outputCandidate : outputCandidates) {
            if (outputCandidate == null || outputCandidate.isEmpty()) {
                continue;
            }
            if (!ItemTest.isOutputStackAllowedForPattern(patternStack, outputCandidate)) {
                continue;
            }
            if (firstValid.isEmpty()) {
                firstValid = outputCandidate;
            }

            if (inputNamespace.isEmpty()) {
                continue;
            }
            String outputNamespace = getRegistryNamespace(outputCandidate);
            if (!inputNamespace.equals(outputNamespace)) {
                continue;
            }

            int score = commonPrefixLength(inputPath, getRegistryPath(outputCandidate));
            if (score > bestScore) {
                bestScore = score;
                bestSameNamespace = outputCandidate;
            }
        }
    ...
    private ItemStack selectFirstAllowedStack(List<ItemStack> stacks, boolean inputSide) {
        ...
        for (ItemStack stack : stacks) {
            ...
            if (!isStackAllowedForSide(stack, inputSide)) {
                continue;
            }
            return stack;
        }
    ...
    private boolean isStackAllowedForSide(ItemStack stack, boolean inputSide) {
        return inputSide
    ...
    private List<ItemStack> buildConcreteVirtualStacks(List<String> oreNames, List<Integer> counts,
                                                       ItemStack selectedPrimary, boolean inputSide) {
        List<ItemStack> stacks = new ArrayList<>();
        ...
        for (int i = 0; i < oreNames.size(); i++) {
            ...
            ItemStack fallback = selectFirstAllowedStack(oreItems, inputSide);
            if (fallback.isEmpty()) {
                return new ArrayList<>();
            }
            ItemStack chosen = (i == 0 && selectedPrimary != null && !selectedPrimary.isEmpty())
                ? selectedPrimary.copy()
                : fallback.copy();
            if (!isStackAllowedForSide(chosen, inputSide)) {
                return new ArrayList<>();
            }
            int count = counts != null && i < counts.size() ? counts.get(i) : 1;
            chosen.setCount(Math.max(1, count));
            stacks.add(chosen);
        }
        return stacks;
    }

    private ItemStack createVirtualPatternStack(List<String> specificInputs,
                                                List<String> specificOutputs,
                                                List<ItemStack> concreteInputs,
                                                List<ItemStack> concreteOutputs,
                                                String virtualDisplayName,
                                                String filterEntryId) {
        ...
        if (filterEntryId != null && !filterEntryId.isEmpty()) {
            ...
            tag.removeTag(TAG_VIRTUAL_FILTER_ENTRY_ID);
        }

        if (concreteInputs != null && !concreteInputs.isEmpty()) {
            tag.setTag(TAG_VIRTUAL_INPUT_STACKS, writeItemStackList(concreteInputs));
        } else {
            tag.removeTag(TAG_VIRTUAL_INPUT_STACKS);
        }
        if (concreteOutputs != null && !concreteOutputs.isEmpty()) {
            tag.setTag(TAG_VIRTUAL_OUTPUT_STACKS, writeItemStackList(concreteOutputs));
        } else {
            tag.removeTag(TAG_VIRTUAL_OUTPUT_STACKS);
        }
    ...
    private static String buildVariantFilterEntryId(String inputOre, String outputOre, ItemStack inputItem, ItemStack outputItem) {
        String legacyId = inputOre + "->" + outputOre;
        return legacyId + "|" + buildItemKey(inputItem) + "=>" + buildItemKey(outputItem);
    }
    ...
    private static List<ItemStack> readItemStackList(NBTTagCompound tag, String key) {
        List<ItemStack> result = new ArrayList<>();
        if (tag == null || key == null || key.isEmpty() || !tag.hasKey(key)) {
            return result;
        }
