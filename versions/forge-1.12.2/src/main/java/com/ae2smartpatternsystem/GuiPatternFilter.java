...
public class GuiPatternFilter extends GuiContainer {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation("sampleintegration", "textures/gui/pattern_all.png");
    ...
    private final Set<String> highlightedEntryIds = new HashSet<>();
    ...
    public void initGui() {
        ...
        this.buttonList.add(new InvisibleButton(BUTTON_SEARCH, guiLeft + 180, guiTop + 50, 20, 20, "S",
            I18n.format("gui.sampleintegration.open_search")));
        this.buttonList.add(new InvisibleButton(BUTTON_MOD_FILTER, guiLeft + 180, guiTop + 74, 20, 20, "M",
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button.id == BUTTON_SEARCH) {
            this.mc.displayGuiScreen(new GuiPatternSearch(this.player, GuiHandler.PATTERN_FILTER_GUI));
            return;
        }
        if (button.id == BUTTON_MOD_FILTER) {
            this.mc.displayGuiScreen(new GuiModFilter(this.player, GuiHandler.PATTERN_FILTER_GUI));
        }
    }
    ...
    private void drawFilterScreen(int mouseX, int mouseY) {
        ...
        for (int row = 0; row < LIST_ROWS; row++) {
            for (int col = 0; col < LIST_COLS; col++) {
                ...
                boolean highlighted = highlightedEntryIds.contains(entry.id);
                boolean selectedEntry = isEntrySelected(selected, entry.id, entry.orePair);
                drawSlotItem(inputX, inputY, entry, true, selectedEntry, mode, highlighted);
                drawSlotItem(outputX, outputY, entry, false, selectedEntry, mode, highlighted);
            }
        }
    ...
    private void handleFilterClick(int mouseX, int mouseY, int mouseButton) {
        ...
        if (mouseX >= inputX && mouseX < inputX + LIST_COLS * SLOT_SIZE && mouseY >= inputY && mouseY < inputY + listHeight) {
            ...
            if (idx >= 0 && idx < recipes.size()) {
                RecipeEntry entry = recipes.get(idx);
                updateFilterSelection(entry.id, entry.orePair, selected, mouseButton);
                return;
            }
        }
    ...
    private List<RecipeEntry> getRecipeEntries() {
        ...
        for (String[] recipe : recipes) {
            ...
            String orePair = input + "->" + output;
            String id = recipe.length > 3 && recipe[3] != null && !recipe[3].isEmpty() ? recipe[3] : orePair;
            VariantItemKeys variantItemKeys = parseVariantItemKeys(id);
            result.add(new RecipeEntry(
                ...
                input,
                output,
                variantItemKeys.inputKey,
                variantItemKeys.outputKey
            ));
        }
        return result;
    }

    private void applyPendingSearchJump() {
        highlightedEntryIds.clear();
        String pendingId = GuiNavigationState.consumePendingFilterEntryId();
        ...
        for (int i = 0; i < recipes.size(); i++) {
            ...
            if (matchesPendingJump(entry, pendingId)) {
        ...
        int anchorIndex = matchedIndices.get(0);
        int targetPage = anchorIndex / 9;
        int maxScroll = Math.max(0, (int) Math.ceil(recipes.size() / 9.0) - 1);
        listScroll = Math.max(0, Math.min(maxScroll, targetPage));

        for (Integer index : matchedIndices) {
            if (index == null) {
                continue;
            }
            if (index / 9 != targetPage) {
                continue;
            }
    ...
    private boolean matchesPendingJump(RecipeEntry entry, String pendingId) {
        if (entry == null || pendingId == null || pendingId.isEmpty()) {
            return false;
        }
        if (pendingId.equals(entry.id) || pendingId.equals(entry.orePair)) {
            return true;
        }
        return entry.id != null && entry.id.startsWith(pendingId + "|");
    }
    ...
    private ItemStack resolveDisplayStackByItemKey(String itemKey) {
        if (itemKey == null || itemKey.isEmpty()) {
            return ItemStack.EMPTY;
        }

        String registryName = itemKey;
        int metadata = 0;
        int split = itemKey.lastIndexOf('@');
        if (split >= 0) {
            registryName = itemKey.substring(0, split);
            String metaPart = itemKey.substring(split + 1);
            try {
                metadata = Integer.parseInt(metaPart);
        ...
        ItemStack stack = new ItemStack(item, 1, Math.max(0, metadata));
        if (stack.isEmpty() && metadata != 0) {
            stack = new ItemStack(item, 1, 0);
        }
        return stack;
    }

    private ItemStack resolveEntryDisplayStack(RecipeEntry entry, boolean inputSide) {
        if (entry == null) {
            return ItemStack.EMPTY;
        }
        String itemKey = inputSide ? entry.inputItemKey : entry.outputItemKey;
        ItemStack byItemKey = resolveDisplayStackByItemKey(itemKey);
        if (!byItemKey.isEmpty()) {
            return byItemKey;
        }
        return resolveDisplayStack(inputSide ? entry.inputOre : entry.outputOre);
    }

    private void drawSlotItem(int x, int y, RecipeEntry entry, boolean inputSide, boolean selected, int mode, boolean highlighted) {
        if (highlighted) {
            drawRect(x, y, x + 16, y + 16, SEARCH_HIGHLIGHT_COLOR);
        }
        ItemStack stack = resolveEntryDisplayStack(entry, inputSide);
        if (!stack.isEmpty()) {
    ...
    private void updateFilterSelection(String id, String legacyId, List<String> selected, int mouseButton) {
        if (id == null || id.isEmpty()) {
            return;
        }
        boolean containsExact = selected != null && selected.contains(id);
        boolean containsLegacy = legacyId != null
            && !legacyId.isEmpty()
            && selected != null
            && selected.contains(legacyId)
            && !hasVariantSelectionForPair(selected, legacyId);
        boolean contains = containsExact || containsLegacy;
        if (mouseButton == 0) {
            if (containsLegacy && !containsExact) {
                PacketHandler.INSTANCE.sendToServer(PacketUpdatePatternFilter.forToggle(legacyId));
                container.toggleFilterEntry(legacyId);
            }
            if (!containsExact) {
            ...
            String removeKey = containsExact ? id : legacyId;
            if (removeKey != null && !removeKey.isEmpty()) {
                PacketHandler.INSTANCE.sendToServer(PacketUpdatePatternFilter.forToggle(removeKey));
                container.toggleFilterEntry(removeKey);
            }
        }
    }

    private boolean isEntrySelected(List<String> selected, String id, String legacyId) {
        if (selected == null || selected.isEmpty()) {
            return false;
        }
        if (id != null && !id.isEmpty() && selected.contains(id)) {
            return true;
        }
        if (legacyId == null || legacyId.isEmpty()) {
            return false;
        }
        if (hasVariantSelectionForPair(selected, legacyId)) {
            return false;
        }
        return selected.contains(legacyId);
    }

    private boolean hasVariantSelectionForPair(List<String> selected, String legacyId) {
        if (selected == null || selected.isEmpty() || legacyId == null || legacyId.isEmpty()) {
            return false;
        }
    ...
    private void appendPatternDetails(List<String> lines, ItemStack patternStack, RecipeEntry entry) {
        ...
        if (lines.isEmpty()) {
            List<String> inputNames = resolveEntryItemNames(entry, true);
            List<String> outputNames = resolveEntryItemNames(entry, false);
    ...
    private void appendEntryItemDetail(List<String> lines, RecipeEntry entry, boolean input, int count) {
        if (lines == null || entry == null) {
            return;
        }
        String key = input ? "tooltip.sampleintegration.input_ore" : "tooltip.sampleintegration.output_ore";
        List<String> names = resolveEntryItemNames(entry, input);
        if (names.isEmpty()) {
            return;
        }
        lines.add(TextFormatting.DARK_GRAY + I18n.format(key, 1, names.get(0), Math.max(1, count)));
    }

    private int resolveEntryCount(List<String> oreNames, List<Integer> counts, String targetOre) {
        if (counts == null || counts.isEmpty()) {
            return 1;
        }
        if (oreNames == null || oreNames.isEmpty() || targetOre == null || targetOre.isEmpty()) {
            return Math.max(1, counts.get(0));
        }
        for (int i = 0; i < oreNames.size(); i++) {
            String configuredOre = oreNames.get(i);
            if (configuredOre == null || configuredOre.isEmpty()) {
                continue;
            }
            if (configuredOre.equals(targetOre)) {
                return i < counts.size() ? Math.max(1, counts.get(i)) : 1;
            }
            if (configuredOre.contains("*")) {
                String prefix = configuredOre.replace("*", "");
                if (!prefix.isEmpty() && targetOre.startsWith(prefix)) {
                    return i < counts.size() ? Math.max(1, counts.get(i)) : 1;
                }
            }
        }
    ...
    private void appendFluidDetails(List<String> lines, List<String> fluids, List<Integer> amounts, boolean input) {
        ...
        TextFormatting color = input ? TextFormatting.DARK_AQUA : TextFormatting.BLUE;
        ...
        for (int i = 0; i < fluids.size(); i++) {
            ...
            lines.add(color + I18n.format(key, i + 1, fluidName, Math.max(1, amount)));
        }
    }
    ...
    private void appendGasDetails(List<String> lines, List<String> gases, List<Integer> amounts, boolean input) {
        ...
        for (int i = 0; i < gases.size(); i++) {
            ...
            lines.add(color + I18n.format(key, i + 1, gasName, Math.max(1, amount)));
        }
    }
    ...
    private List<String> resolveEntryItemNames(RecipeEntry entry, boolean inputSide) {
        List<String> names = new ArrayList<>();
        if (entry == null) {
            return names;
        }
        ItemStack displayStack = resolveEntryDisplayStack(entry, inputSide);
        if (!displayStack.isEmpty()) {
            names.add(displayStack.getDisplayName());
            return names;
        }
        return resolveItemNames(inputSide ? entry.inputOre : entry.outputOre);
    }
    ...
    private VariantItemKeys parseVariantItemKeys(String entryId) {
        if (entryId == null || entryId.isEmpty()) {
            return VariantItemKeys.EMPTY;
        }
        int variantSplit = entryId.indexOf('|');
        if (variantSplit < 0 || variantSplit >= entryId.length() - 1) {
            return VariantItemKeys.EMPTY;
        }
        int arrowSplit = entryId.indexOf("=>", variantSplit + 1);
        if (arrowSplit < 0) {
            return VariantItemKeys.EMPTY;
        }
        String inputKey = entryId.substring(variantSplit + 1, arrowSplit).trim();
        String outputKey = entryId.substring(arrowSplit + 2).trim();
        if (inputKey.isEmpty() || outputKey.isEmpty()) {
            return VariantItemKeys.EMPTY;
        }
        return new VariantItemKeys(inputKey, outputKey);
    }

    private static class RecipeEntry {
        final String id;
        final String orePair;
        final String displayName;
        final String inputOre;
        final String outputOre;
        final String inputItemKey;
        final String outputItemKey;

        RecipeEntry(String id, String orePair, String displayName, String inputOre, String outputOre,
                    String inputItemKey, String outputItemKey) {
            this.id = id;
            this.orePair = orePair;
    ...
    private static class VariantItemKeys {
        static final VariantItemKeys EMPTY = new VariantItemKeys("", "");
        final String inputKey;
        final String outputKey;

        VariantItemKeys(String inputKey, String outputKey) {
