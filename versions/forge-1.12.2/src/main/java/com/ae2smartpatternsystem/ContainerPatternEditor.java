...
public class ContainerPatternEditor extends Container {
    ...
    private void restoreEditorStateFromPattern() {
        if (this.patternStack.isEmpty() || !(this.patternStack.getItem() instanceof ItemTest) || !this.patternStack.hasTagCompound()) {
            return;
        }

        NBTTagCompound tag = this.patternStack.getTagCompound();
        boolean restored = restorePreviewSlots(tag, TAG_EDITOR_INPUT_SLOTS, this.inputInventory, INPUT_SLOTS);
        restored |= restorePreviewSlots(tag, TAG_EDITOR_OUTPUT_SLOTS, this.outputInventory, OUTPUT_SLOTS);

        if (!restored) {
            restoreEditorStateFromEncodedPattern();
        }
    }

    private boolean restorePreviewSlots(NBTTagCompound tag, String listKey, IInventory inventory, int maxSlots) {
        if (tag == null || inventory == null || listKey == null || listKey.isEmpty() || !tag.hasKey(listKey, 9)) {
            return false;
        }
        ...
        for (int i = 0; i < list.tagCount(); i++) {
            ...
            int slot = entry.getInteger(TAG_EDITOR_SLOT);
            if (slot < 0 || slot >= maxSlots) {
                continue;
            }

            ItemStack restoredStack = new ItemStack(entry.getCompoundTag(TAG_EDITOR_STACK));
            if (restoredStack.isEmpty()) {
                continue;
            }

            inventory.setInventorySlotContents(slot, restoredStack);
            restored = true;
        }
        return restored;
    }

    private void restoreEditorStateFromEncodedPattern() {
        int inputSlot = 0;
        inputSlot = restoreOreMarkers(
            ItemTest.getInputOreNamesStatic(this.patternStack),
            ItemTest.getInputCountsStatic(this.patternStack),
            this.inputInventory,
            inputSlot,
            INPUT_SLOTS
        );
        inputSlot = restoreFluidMarkers(
            ItemTest.getInputFluidsStatic(this.patternStack),
            ItemTest.getInputFluidAmountsStatic(this.patternStack),
            this.inputInventory,
            inputSlot,
            INPUT_SLOTS
        );
        restoreGasMarkers(
            ItemTest.getInputGasesStatic(this.patternStack),
            ItemTest.getInputGasAmountsStatic(this.patternStack),
            ItemTest.getInputGasItemsStatic(this.patternStack),
            this.inputInventory,
            inputSlot,
            INPUT_SLOTS
        );
        ...
        int outputSlot = 0;
        outputSlot = restoreOreMarkers(
            ItemTest.getOutputOreNamesStatic(this.patternStack),
            ItemTest.getOutputCountsStatic(this.patternStack),
            this.outputInventory,
            outputSlot,
            OUTPUT_SLOTS
        );
        outputSlot = restoreFluidMarkers(
            ItemTest.getOutputFluidsStatic(this.patternStack),
            ItemTest.getOutputFluidAmountsStatic(this.patternStack),
            this.outputInventory,
            outputSlot,
            OUTPUT_SLOTS
        );
        restoreGasMarkers(
            ItemTest.getOutputGasesStatic(this.patternStack),
            ItemTest.getOutputGasAmountsStatic(this.patternStack),
            ItemTest.getOutputGasItemsStatic(this.patternStack),
            this.outputInventory,
            outputSlot,
            OUTPUT_SLOTS
        );
    ...
    private int restoreOreMarkers(List<String> oreNames, List<Integer> counts, IInventory inventory, int startSlot, int maxSlots) {
        int slot = startSlot;
        if (oreNames == null) {
            return slot;
        }
        for (int i = 0; i < oreNames.size() && slot < maxSlots; i++) {
            String oreName = oreNames.get(i);
            if (oreName == null || oreName.isEmpty() || oreName.contains("*")) {
                continue;
            }
            ...
            int amount = i < counts.size() ? counts.get(i) : 1;
            ItemStack marker = createItemMarkerStack(source, Math.max(1, amount));
            if (marker.isEmpty()) {
                continue;
            }
            inventory.setInventorySlotContents(slot++, marker);
        }
        return slot;
    }

    private int restoreFluidMarkers(List<String> fluidNames, List<Integer> amounts, IInventory inventory, int startSlot, int maxSlots) {
        int slot = startSlot;
        if (fluidNames == null) {
            return slot;
        }
        for (int i = 0; i < fluidNames.size() && slot < maxSlots; i++) {
            String fluidName = fluidNames.get(i);
            if (fluidName == null || fluidName.isEmpty()) {
                continue;
            }
            int amount = i < amounts.size() ? amounts.get(i) : DEFAULT_FLUID_AMOUNT;
            ItemStack marker = createFluidMarkerStack(fluidName, Math.max(1, amount));
            if (marker.isEmpty()) {
                continue;
            }
            inventory.setInventorySlotContents(slot++, marker);
    ...
    private int restoreGasMarkers(List<String> gasNames, List<Integer> amounts, List<ItemStack> gasItems, IInventory inventory, int startSlot, int maxSlots) {
        int slot = startSlot;
        if (gasNames == null) {
            return slot;
        }
        for (int i = 0; i < gasNames.size() && slot < maxSlots; i++) {
            String gasName = gasNames.get(i);
            if (gasName == null || gasName.isEmpty()) {
                continue;
            }
            int amount = i < amounts.size() ? amounts.get(i) : DEFAULT_GAS_AMOUNT;
            ItemStack source = (gasItems != null && i < gasItems.size()) ? gasItems.get(i) : ItemStack.EMPTY;
            ItemStack marker = !source.isEmpty()
                ? createGasMarkerStack(source, new GasInfo(gasName, Math.max(1, amount)))
                : createGasMarkerStack(gasName, Math.max(1, amount));
            if (marker.isEmpty()) {
                continue;
            }
            inventory.setInventorySlotContents(slot++, marker);
    ...
    private void persistEditorPreviewStacks(ItemStack patternStack) {
        ...
        writePreviewSlots(tag, TAG_EDITOR_INPUT_SLOTS, this.inputInventory, INPUT_SLOTS);
        writePreviewSlots(tag, TAG_EDITOR_OUTPUT_SLOTS, this.outputInventory, OUTPUT_SLOTS);
        writePreviewSlots(tag, TAG_INPUTS, this.inputInventory, INPUT_SLOTS);
        writePreviewSlots(tag, TAG_OUTPUTS, this.outputInventory, OUTPUT_SLOTS);
    }

    private void writePreviewSlots(NBTTagCompound tag, String listKey, IInventory inventory, int maxSlots) {
        ...
        for (int i = 0; i < maxSlots; i++) {
            ...
            entry.setInteger(TAG_EDITOR_SLOT, i);
            NBTTagCompound stackTag = new NBTTagCompound();
            stack.writeToNBT(stackTag);
            entry.setTag(TAG_EDITOR_STACK, stackTag);
            list.appendTag(entry);
        }
        if (list.tagCount() > 0) {
            tag.setTag(listKey, list);
        } else if (tag.hasKey(listKey)) {
            tag.removeTag(listKey);
        }
    }
    ...
    public List<String[]> getAvailableRecipeTypes() {
        ...
        List<DerivedRecipeOption> derivedRecipes = deriveRecipes(inputStack, outputStack);

        for (int i = 0; i < derivedRecipes.size(); i++) {
            DerivedRecipeOption recipe = derivedRecipes.get(i);
            String inputName = recipe.inputStack.getDisplayName();
            String outputName = recipe.outputStack.getDisplayName();
            String displayName = inputName + " -> " + outputName;
            recipeTypes.add(new String[]{recipe.inputOreName, recipe.outputOreName, displayName});
        }
    ...
    private void encodeSelectedPattern() {
        ...
        String selectedInputOre = selectedRecipe[0];
        String selectedOutputOre = selectedRecipe[1];
        String selectedInputMaterial = extractMaterial(selectedInputOre);
        String selectedOutputMaterial = extractMaterial(selectedOutputOre);
        boolean useWildcardPair = selectedInputMaterial != null
            && selectedInputMaterial.equals(selectedOutputMaterial);
        ...
        if (!inputFluids.isEmpty() || !outputFluids.isEmpty() || !inputGases.isEmpty() || !outputGases.isEmpty()) {
            ...
            outputCounts.add(outputCount);
                ...
                persistEditorPreviewStacks(patternStack);
        } else {
    ...
    /**
     */
    private List<DerivedRecipeOption> deriveRecipes(ItemStack inputStack, ItemStack outputStack) {
        List<DerivedRecipeOption> derivedRecipes = new ArrayList<>();

        List<String> inputOreNames = getOreNames(inputStack);
        List<String> outputOreNames = getOreNames(outputStack);
        ...
        TechStart.LOGGER.info(
            ...
            inputOreNames,
            describeStack(outputStack),
            outputOreNames,
            anchor == null ? "null" : (anchor.inputOreName + " -> " + anchor.outputOreName)
        );
        if (anchor == null) {
            return derivedRecipes;
        }

        derivedRecipes.add(new DerivedRecipeOption(
            inputStack.copy(),
            outputStack.copy(),
            anchor.inputOreName,
            anchor.outputOreName
        ));

        if (!anchor.sameMaterial) {
            return derivedRecipes;
        }

        String inputBaseType = anchor.inputBaseType;
        String outputBaseType = anchor.outputBaseType;
        String inputMaterial = anchor.inputMaterial;
        String outputMaterial = anchor.outputMaterial;
        ...
        for (String material : inputMaterials) {
            ...
            if (material.equals(inputMaterial) && material.equals(outputMaterial)) {
                continue;
            }
    ...
    private CandidatePair findBestCandidatePair(ItemStack inputStack, ItemStack outputStack, List<String> inputOreNames, List<String> outputOreNames) {
        CandidatePair fallback = null;
        CandidatePair best = null;
        int bestScore = Integer.MIN_VALUE;
        for (String inputOreName : inputOreNames) {
            String inputBaseType = extractBaseType(inputOreName);
            String inputMaterial = extractMaterial(inputOreName);
            if (inputBaseType == null || inputMaterial == null) {
                continue;
            }
            for (String outputOreName : outputOreNames) {
                String outputBaseType = extractBaseType(outputOreName);
                String outputMaterial = extractMaterial(outputOreName);
                if (outputBaseType == null || outputMaterial == null) {
                    continue;
                }
                ...
                if (inputMaterial.equals(outputMaterial)) {
                    CandidatePair matched = pair.withSameMaterial(true);
                    int score = scoreCandidatePair(inputStack, outputStack, matched);
                ...
                if (fallback == null) {
                    fallback = pair.withSameMaterial(false);
                }
            }
        }
    ...
    private int scoreCandidatePair(ItemStack inputStack, ItemStack outputStack, CandidatePair pair) {
        int score = 0;
        String material = pair.inputMaterial == null ? "" : pair.inputMaterial.toLowerCase(Locale.ROOT);
        if (material.isEmpty()) {
            return score;
        }

        String inputPath = getRegistryPath(inputStack).toLowerCase(Locale.ROOT);
        String outputPath = getRegistryPath(outputStack).toLowerCase(Locale.ROOT);
        String inputDisplay = inputStack.isEmpty() ? "" : inputStack.getDisplayName().toLowerCase(Locale.ROOT);
        String outputDisplay = outputStack.isEmpty() ? "" : outputStack.getDisplayName().toLowerCase(Locale.ROOT);

        if (inputPath.contains(material)) {
            score += 4;
        }
        if (outputPath.contains(material)) {
            score += 4;
        }
        if (inputDisplay.contains(material)) {
            score += 2;
        }
        if (outputDisplay.contains(material)) {
            score += 2;
        }
        if (pair.inputOreName.toLowerCase(Locale.ROOT).contains(material)) {
            score += 1;
        }
        if (pair.outputOreName.toLowerCase(Locale.ROOT).contains(material)) {
            score += 1;
        }
        return score;
    }
    ...
    private String describeStack(ItemStack stack) {
        ...
        return registry + " | " + stack.getDisplayName();
    }
    ...
    private List<String> getOreNames(ItemStack stack) {
        ...
        for (int oreId : oreIds) {
            ...
            if (oreName != null && !oreName.isEmpty() && !result.contains(oreName)) {
                result.add(oreName);
            }
        }
        return result;
    ...
    private static final class CandidatePair {
        ...
        private final String inputBaseType;
        private final String outputBaseType;
        private final String inputMaterial;
        private final String outputMaterial;
        private final boolean sameMaterial;

        private CandidatePair(String inputOreName, String outputOreName, String inputBaseType,
                              String outputBaseType, String inputMaterial, String outputMaterial) {
            this(inputOreName, outputOreName, inputBaseType, outputBaseType, inputMaterial, outputMaterial, false);
        }

        private CandidatePair(String inputOreName, String outputOreName, String inputBaseType,
                              String outputBaseType, String inputMaterial, String outputMaterial, boolean sameMaterial) {
            this.inputOreName = inputOreName;
            this.outputOreName = outputOreName;
            this.inputBaseType = inputBaseType;
            this.outputBaseType = outputBaseType;
            this.inputMaterial = inputMaterial;
            this.outputMaterial = outputMaterial;
            this.sameMaterial = sameMaterial;
        }

        private CandidatePair withSameMaterial(boolean value) {
    ...
    private static final class DerivedRecipeOption {
        private final ItemStack inputStack;
        private final ItemStack outputStack;
        private final String inputOreName;
        private final String outputOreName;

        private DerivedRecipeOption(ItemStack inputStack, ItemStack outputStack, String inputOreName, String outputOreName) {
    ...
    private void encodePatternOnClose() {
        ...
        List<String[]> availableTypes = getAvailableRecipeTypes();
        if (!availableTypes.isEmpty() && selectedRecipeType >= 0 && selectedRecipeType < availableTypes.size()) {
            encodeSelectedPattern();
            return;
        }
    ...
    private class DerivedRecipeInventory implements IInventory {
        ...
        @Override
        public ItemStack getStackInSlot(int index) {
            List<DerivedRecipeOption> derivedRecipes = deriveRecipes(getInputStack(), getOutputStack());
            if (index < derivedRecipes.size()) {
                DerivedRecipeOption recipe = derivedRecipes.get(index);
                return index == 0 ? recipe.inputStack : recipe.outputStack;
            }
            return ItemStack.EMPTY;
        }
