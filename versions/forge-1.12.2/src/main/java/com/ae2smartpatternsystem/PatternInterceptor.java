...
public class PatternInterceptor {
    ...
    public static boolean interceptAndExpand(DualityInterface duality, ItemStack stack) {
        ...
        try {
            if (craftingListField == null) {
                return false;
            }

            Set<ICraftingPatternDetails> craftingList = (Set<ICraftingPatternDetails>) craftingListField.get(duality);
            if (craftingList == null) {
                craftingList = new HashSet<>();
                craftingListField.set(duality, craftingList);
            }

            craftingList.removeIf(pattern -> isInjectedVirtualPatternForStack(pattern, stack));

            List<SmartPatternDetails> expanded = mainPattern.expandToVirtualPatterns();
            for (SmartPatternDetails detail : expanded) {
                if (detail != null && detail.isAllowedByFilter()) {
                    craftingList.add(detail);
                }
            }
            return true;
        } catch (Exception e) {
            System.err.println("[PatternInterceptor] Failed to inject wildcard patterns into DualityInterface: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }

    private static boolean isInjectedVirtualPatternForStack(ICraftingPatternDetails pattern, ItemStack sourceStack) {
        if (!(pattern instanceof SmartPatternDetails) || sourceStack == null || sourceStack.isEmpty()) {
            return false;
        }
        SmartPatternDetails smart = (SmartPatternDetails) pattern;
        if (!smart.isVirtual()) {
            return false;
        }

        ItemStack existingPattern = pattern.getPattern();
        if (existingPattern == null || existingPattern.isEmpty()) {
            return false;
        }
        if (existingPattern.getItem() != sourceStack.getItem()) {
            return false;
        }
        if (existingPattern.getMetadata() != sourceStack.getMetadata()) {
            return false;
        }

        NBTTagCompound existingTag = sanitizeVirtualTag(existingPattern.getTagCompound());
        NBTTagCompound sourceTag = sanitizeVirtualTag(sourceStack.getTagCompound());
        if (existingTag == null || existingTag.isEmpty()) {
            return sourceTag == null || sourceTag.isEmpty();
        }
        return existingTag.equals(sourceTag);
    }

    private static NBTTagCompound sanitizeVirtualTag(NBTTagCompound original) {
        if (original == null) {
            return null;
        }
    ...
    public static boolean interceptMMCERefreshPattern(Object provider, int slot) {
        ...
        try {
            ...
            ICraftingPatternDetails detail = ((ItemTest) item).getPatternForItem(pattern, world);
            if (detail == null) {
                return false;
            }
